<?php
namespace Perrystown\App\Gallery;

if (!defined('ABSPATH')) exit;

class Gallery_Controller {

    /** GET /galleries — list + pagination + multi-word search */
    public static function index(\WP_REST_Request $request) {
        global $wpdb; $table = Gallery_Table::table_name();
        ...
        if ($search !== '') {
            ...
            foreach ($tokens as $tok) {
                ...
                $parts[] = '(name LIKE %s OR title LIKE %s)';
        ...
        $rows_sql  = "SELECT id, name, title, image, created_at
                      FROM {$table}
                      WHERE {$where_sql}
    ...
    /** POST /galleries — real file upload OR URL */
    public static function store(\WP_REST_Request $request) {
        global $wpdb; $table = Gallery_Table::table_name();

        // Get sanitized params directly (validator already sanitized them)
        $name  = $request->get_param('name');
        $title = $request->get_param('title');

        // image: file first, otherwise URL string
        ...
        if ($ok === false) {
            return new \WP_REST_Response(['status'=>false,'message'=>'Failed to save gallery item.'], 500);
        }
    ...
    public static function show(\WP_REST_Request $request) {
        ...
        $row = $wpdb->get_row(
            $wpdb->prepare("SELECT id, name, title, image, created_at FROM {$table} WHERE id = %d", $id),
            ARRAY_A
        );
        if (!$row) return new \WP_REST_Response(['status'=>false,'message'=>'Gallery item not found.'], 404);

        return new \WP_REST_Response([
            'status'=>true,'message'=>'Gallery item fetched successfully.','data'=>$row
        ], 200);
    }

    /** PUT/PATCH/POST /galleries/{id} — partial update; POST supports multipart image file */
    public static function update(\WP_REST_Request $request) {
        ...
        // Accept normal text fields from request (validator already sanitized)
        foreach (['name'=>'%s','title'=>'%s','image'=>'%s'] as $field => $fmt) {
        ...
        return new \WP_REST_Response([
            'status'=>true,'message'=>'Gallery item updated successfully.','data'=>['id'=>$id]
        ], 200);
    }
    ...
    public static function destroy(\WP_REST_Request $request) {
        ...
        if (!$deleted) {
            return new \WP_REST_Response(['status'=>false,'message'=>'Delete failed or gallery item not found.'], 400);
        }
        return new \WP_REST_Response(['status'=>true,'message'=>'Gallery item deleted successfully.'], 200);
    }
}